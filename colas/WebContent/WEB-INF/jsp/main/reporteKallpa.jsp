...
<!--               <li class="breadcrumb-item"><a style="color:#007bff; cursor:pointer" id="volverSucursal">Sucursal</a></li> -->
              ...
              <li class="breadcrumb-item active">Reporte de Kallpa</li>
            </ol>
          </div>
        </div>
      </div>
        	...
        	<div class="col-12">
          		<div class="card">
            		<div class="card-header">
            			<h3 class="card-title">Reporte de Kallpa</h3>
            		</div>
            		<div class="card-body">
              			<div class="row">
                            <div class="col-sm-6">
           		...
           		<div class="card">
            		<div id="reporteKallpa"  class="card-body">
	            		<!--cambio VBR 20191120-->
            			<div class="table-responsive">
	            			<table id="tblListadoKallpa" class="table table-bordered table-striped">
				                <thead>
					                <tr>
					                	<th>ID Ticket</th>
					                    <th>Oficina</th>
...
function Construyecabecera(presentacion){
	...
	$('#tblListadoKallpa thead').empty();	
		...
		html += '<th>Grupo Económico</th>';
		html += '<th>Producto</th>';
		html += '<th>N° de Poliza</th>';
		html += '<th>Estado</th>';
		html += '<th>Motivo</th>';
		html += '<th>Fecha Caso</th>';
		html += '<th>Caso SF</th>';
		html += '<th>Trámite</th>';
		html += '<th>Fecha Trámite</th>';
		html += '<th>Ejecutivo</th>';
		html += '<th>Comentario</th>';
		html += '<th>Tipo Gestión</th>';
		html += '<th>Ubigeo</th>';
		html += '<th>Zona Ubigeo</th>';
		html += '<th>Mes</th>';
		html += '<th>Estado 2</th>';
		html += '<th>Prima Neta</th>';
		html += '<th>Año</th>';
		html += '<th>Derivado</th>';
		html += '<th>Priorizado</th>';
		html += '<th class="noExl">Audio</th>';
		console.log(html);
		$('#tblListadoKallpa thead').html('<tr>' + html + '</tr>');
}

function Construyehtml(data) {
	if ($.fn.DataTable.isDataTable('#tblListadoKallpa')) {
		$('#tblListadoKallpa').DataTable().clear().destroy();
	}
	
    $('#tblListadoKallpa tbody').empty();
    ...
    for (var i = 0; i < data.length; i++) {
    	...
    	if (data[i].idAtencionKallpa == null) {
        ...
        html = '<tr>' +
            ...
            '<td><span>' + data[i].idAtencionKallpa + '</span></td>' +
            ...
            '<td><span>' + data[i].tipoCliente + '</span></td>' +
            '<td><span>' + data[i].grupoEconomico + '</span></td>' +
            '<td><span>' + data[i].producto + '</span></td>' +
            '<td><span>' + data[i].numeroPoliza + '</span></td>' +
            '<td><span>' + data[i].estado + '</span></td>' +
            '<td><span>' + data[i].motivoAnulacion + '</span></td>' +
            '<td><span>' + data[i].fechaCaso + '</span></td>' +
            '<td><span>' + data[i].casoSF + '</span></td>' +
            '<td><span>' + data[i].numeroTramite + '</span></td>' +
            '<td><span>' + data[i].fechaTramite + '</span></td>' +
            '<td><span>' + data[i].ejecutivo + '</span></td>' +
            '<td><span>' + data[i].comentario + '</span></td>' +
            '<td><span>' + data[i].tipoGestion + '</span></td>' +
            '<td><span>' + data[i].ubigeo + '</span></td>' +
            '<td><span>' + data[i].zonaUbigeo + '</span></td>' +
            '<td><span>' + data[i].mes + '</span></td>' +
            '<td><span>' + data[i].estado2 + '</span></td>' +
            '<td><span>' + data[i].primaNeta + '</span></td>' +
            '<td><span>' + data[i].anio + '</span></td>' +
            '<td><span>' + data[i].derivado + '</span></td>' +
            '<td><span>' + data[i].priorizado + '</span></td>' +
            audio +
        '</tr>';
        
        $('#tblListadoKallpa tbody').append(html);
...
$("#btnExportArea").click(function() {
	var table = $('#tblListadoKallpa').DataTable();
    var currentLength = table.page.len();

    table.page.len(-1).draw();

    setTimeout(function() {
        $('#tblListadoKallpa').table2excel({
        	exclude: ".noExl",
    		name: "ReporteAtencionesKallpa",
    		filename: "ReporteAtencionesKallpa.xls",
...
$("#btnImprimirReporte1").click(function() {
	var printContents = document.getElementById('tblListadoKallpa');
    w = window.open();
