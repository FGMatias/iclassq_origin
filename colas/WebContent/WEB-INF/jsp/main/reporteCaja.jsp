        	...
        	<div class="col-12">
          		<div class="card">
            		<div class="card-header">
            			<h3 class="card-title">Reporte de Caja</h3>
            		</div>
            		<div class="card-body">
              			<div class="row">
                            <div class="col-sm-6">
           		...
           		<div class="card">
            		<div id="reporteCaja"  class="card-body">
	            		<!--cambio VBR 20191120-->
            			<div class="table-responsive">
	            			<table id="tblListadoCaja" class="table table-bordered table-striped">
				                <thead>
					                <tr>
					                    <th>ID Ticket</th>
					                    <th>ID Turno</th>
...
function Construyecabecera(presentacion){
	var	html='';
	$('#tblListadoCaja thead').empty();	
		html += '<th>ID Ticket</th>';
		html += '<th>ID Turno</th>';
		html += '<th>Ticket</th>';
		html += '<th>Sucursal</th>';
		html += '<th>Estado Ticket</th>';
		html += '<th>Proceso Turno</th>';
		html += '<th>Proceso Atención</th>';
		html += '<th>Ofrecimiento</th>';
		...
		console.log(html);
		$('#tblListadoCaja thead').html('<tr>' + html + '</tr>');
}

function Construyehtml(data) {
	if ($.fn.DataTable.isDataTable('#tblListadoCaja')) {
		$('#tblListadoCaja').DataTable().clear().destroy();
	}
	
    $('#tblListadoCaja tbody').empty();
    var html = '';
    
    for (var i = 0; i < data.length; i++) {
		var audio;
    	
		if (data[i].idAtencionCaja == null) {
        ...
        $('#tblListadoCaja tbody').append(html);
...
$("#btnExportArea").click(function() {
	var table = $('#tblListadoCaja').DataTable();
    var currentLength = table.page.len();

    table.page.len(-1).draw();

    setTimeout(function() {
        $('#tblListadoCaja').table2excel({
        	exclude: ".noExl",
    		name: "ReporteAtencionesCaja",
    		filename: "ReporteAtencionesCaja.xls",
...
$("#btnImprimirReporte1").click(function() {
	...
	var printContents = document.getElementById('tblListadoCaja');
    w = window.open();
