...
<!--               <li class="breadcrumb-item"><a style="color:#007bff; cursor:pointer" id="volverSucursal">Sucursal</a></li> -->
              ...
              <li class="breadcrumb-item active">Reporte de Siniestro</li>
            </ol>
          </div>
        </div>
      </div>
        	...
        	<div class="col-12">
          		<div class="card">
            		<div class="card-header">
            			<h3 class="card-title">Reporte de Siniestro</h3>
            		</div>
            		<div class="card-body">
              			<div class="row">
                            <div class="col-sm-6">
           		...
           		<div class="card">
            		<div id="reporteSiniestro"  class="card-body">
	            		<!--cambio VBR 20191120-->
            			<div class="table-responsive">
	            			<table id="tblListadoSiniestro" class="table table-bordered table-striped">
				                <thead>
					                <tr>
					                	<th>ID Ticket</th>
					                    <th>ID Turno</th>
...
function Construyecabecera(presentacion){
	var	html='';
	$('#tblListadoSiniestro thead').empty();	
		html += '<th>ID Ticket</th>';
		html += '<th>ID Turno</th>';
		html += '<th>Ticket</th>';
		html += '<th>Sucursal</th>';
		html += '<th>Estado Ticket</th>';
		html += '<th>Proceso Atención</th>';
		...
		html += '<th>Teléfono</th>';
		html += '<th>Correo</th>';
		html += '<th>Siniestro</th>';
		html += '<th>Subprocesos</th>';
		...
		console.log(html);
		$('#tblListadoSiniestro thead').html('<tr>' + html + '</tr>');
}
function Construyehtml(data) {
	if ($.fn.DataTable.isDataTable('#tblListadoSiniestro')) {
		$('#tblListadoSiniestro').DataTable().clear().destroy();
	}
	
    $('#tblListadoSiniestro tbody').empty();
    ...
    for (var i = 0; i < data.length; i++) {
		...
		if (data[i].idAtencionSiniestro == null) {
        ...
        html = '<tr>' +
            ...
            '<td><span>' + data[i].siniestro + '</span></td>' +
            '<td><span>' + data[i].subprocesos + '</span></td>' +
            '<td><span>' + data[i].importe + '</span></td>' +
        ...
        $('#tblListadoSiniestro tbody').append(html);
...
$("#btnBuscarXArea").click(function() {
	...
	$.ajax({
		type: 'POST', 
		url: 'buscarXAtencionSiniestro.app',
		traditional: true,
    	data: {
...
$("#btnDownloadAudio").click(function() {
	...
	$.ajax({
		type: 'POST', 
		url: 'downloadAudiosSiniestro.app',
		traditional: true,
    	data: { 
    		sucursales: idSucursal,
    		fechaInicioAtencion: fechaInicioAtencion, 
...
$("#btnExportArea").click(function() {
	var table = $('#tblListadoSiniestro').DataTable();
    var currentLength = table.page.len();

    table.page.len(-1).draw();

    setTimeout(function() {
        $('#tblListadoSiniestro').table2excel({
        	exclude: ".noExl",
    		name: "ReporteAtencionesSiniestro",
    		filename: "ReporteAtencionesSiniestro.xls",
...
$("#btnImprimirReporte1").click(function() {
	...
	var printContents = document.getElementById('tblListadoSiniestro');
    w = window.open();
